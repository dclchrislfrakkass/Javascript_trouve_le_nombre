...
if (nombreMax <= 10) {
    nbCoups = 3;
    console.log("vous avez droit à" +nbCoups, " coups max !");
}
else if (nombreMax <= 50) {
    nbCoups = 8;
    console.log("vous avez droit à" +nbCoups, " coups max !");
}
...
for (var i=0; i <nbCoups; i++) {    
    ...
    while (nombre < 0 || nombre> nombreMax ){
        console.log("entrez un chiffre entre 0 et " + nombreMax, ", merci");
        nombre = parseInt(prompt("Entrez un nombre !"));
    }
    compteur++;
    //affiche le compteur pour voir
    console.log("le compteur tourne, déjà : " +compteur, " /" +nbCoups);
...
if (i == nbCoups){
    alert("NOOB!!!")
    console.log("Vous avez fait le nombre max de coups! Vous pouvez recommencer! ---------------Le bon chiffre était " +nombreHasard)
    // break;
}
